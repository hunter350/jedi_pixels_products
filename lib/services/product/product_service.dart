...
class ProductService {
  ...
  getProduct(String authToken) async {
    if (authToken.isNotEmpty) {
      Uri url = Uri.https(
          ApiServiceValues.productsBaseUrl, ApiServiceValues.authBaseUrlPath, {
        'limit': '10',
      ...
      await http.get(
        ...
        if (response.statusCode == 200) {
          //Three ways to parse json. Default, via compute, and via Isolate.spawn - message

          //1. Default
          productListService.isProductLoading = true;
          final productsListModel =
              ProductsListModel.fromRawJson(response.body);
...
class ProductsServiceBackgroundMessage {
  ...
  const ProductsServiceBackgroundMessage(
      {required this.sendPort, required this.encodedJson});
}
...
class ProductsServiceBackgroundParser {
  ...
  Future<ProductsListModel> parseViaIsolate(String encodedJson) async {
    ...
    ProductsServiceBackgroundMessage productsServiceBackgroundMessage =
        ProductsServiceBackgroundMessage(
            sendPort: receivePort.sendPort, encodedJson: encodedJson);

    await Isolate.spawn(
        _fromRawJsonViaIsolate, productsServiceBackgroundMessage);
  ...
  void _fromRawJsonViaIsolate(
      ProductsServiceBackgroundMessage productsServiceBackgroundMessage) async {
